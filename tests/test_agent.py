...
def test_agent_episode_logs_and_replays(fly):
    ...
    o2 = ag.run(
        Features("did:plc:b", -0.7, True, 0), t0 + 3600, "at://did:plc:b/app.bsky.feed.post/1"
    )
    pid = ag.apply_outcome(o2.episode_id, "punishment", "test", "did:plc:b", "at://x/1", t0 + 7200)
    assert pid is not None
    o3 = ag.run(
        Features("did:plc:b", -0.7, True, 1), t0 + 10800, "at://did:plc:b/app.bsky.feed.post/2"
    )
    ...
    assert (
        L.episode(o3.episode_id)["weight_digest_before"]
        != L.episode(o2.episode_id)["weight_digest_after"]
    )
