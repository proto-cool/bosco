...
```
systemctl --user enable --now bosco-nightly.timer bosco-deadman.timer
systemctl --user list-timers | grep bosco
```

The two container units are generated by quadlet and cannot be enabled;
they start at boot through their `[Install]` line plus linger (§1a).
`loginctl show-user nick | grep Linger` must say `yes`. Test the
...
```
systemctl --user start bosco-panel
