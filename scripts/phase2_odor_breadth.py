    ...
    for k in (10, 15, 20, 27):
        for r in (100.0, 150.0):
            ...
            for rep in range(4):
                ...
                mb.append(c1[mbon].mean() * 2); post.append(int(c3.sum())); act.append(int((c1 > 0).sum()))
            jac = np.mean([len(sets[i] & sets[j]) / max(1, len(sets[i] | sets[j])) for i in range(4) for j in range(i + 1, 4)])
            print(f"k{k:2d}@{r:.0f}: KC {np.mean(fr):.3f} (min {min(fr):.3f} max {max(fr):.3f}) J {jac:.2f} act {int(np.mean(act))} MBON {np.mean(mb):5.1f}Hz post {int(np.mean(post))}", flush=True)
    return 0
