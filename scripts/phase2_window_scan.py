...
def flywire():
    ...
    n = len(ids)
    indptr, indices, cnt = csr_from_edges(n, con["Presynaptic_Index"].to_numpy(), con["Postsynaptic_Index"].to_numpy(),
                                          con["Excitatory x Connectivity"].to_numpy().astype(float))
    fw = pd.read_csv(paths.FLYWIRE_ANNOTATIONS, sep="\t", usecols=["root_id", "cell_class", "cell_type", "cell_sub_class"], low_memory=False)
    ix = pd.Index(ids)
    ...
    o = ix.get_indexer(orn.root_id.to_numpy()); s = ix.get_indexer(sugar.root_id.to_numpy())
    mn9 = ix.get_indexer([720575940660219265])
    return n, indptr, indices, cnt, o[o >= 0].astype(np.int32), s[s >= 0].astype(np.int32), mn9[mn9 >= 0].astype(np.int32)


def malecns():
    from bosco.model import load_or_build
    b = load_or_build()
    return (b.n, b.indptr, b.indices, b.count.astype(float) * b.sign,
            b.index_of_present(pop.orns()["bodyId"]), b.index_of_present(pop.grns("sugar/water")),
...
def main() -> int:
    ...
    for w_syn in ws:
        ...
        out = []
        for name, idx, rate in (("sugar", sugar_i, 200.0), ("orn25", orn25, 50.0), ("orn400", orn400, 50.0), ("one", one, 50.0)):
