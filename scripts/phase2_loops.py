...
def battery(net, p, b, label, sugar, bitter, mn9, orn_i, odors, kc, mbon):
    ...
    out = []
    for name, idx, rate in (("sugar", sugar, 200.0), ("bitter", bitter, 200.0),
    ...
    for j, idx in enumerate(odors):
        ...
        sets.append(s); fr.append(len(s) / len(kc)); mb.append(c1[mbon].mean() / 0.3); post.append(int(c3.sum()))
    jac = np.mean([len(sets[i] & sets[j]) / max(1, len(sets[i] | sets[j])) for i in range(len(odors)) for j in range(i + 1, len(odors))])
...
def main() -> int:
    ...
    is_kc = np.zeros(b.n, bool); is_kc[kc] = True
    ...
    print(f"KC->KC edges {kk.sum()}, synapses {kk_syn}; per-KC in-synapses from KCs: mean {indeg.mean():.0f}, max {indeg.max():.0f}")
    ...
    w0 = cnt * p.w_syn
    battery(Net(b.indptr, b.indices, w0, p), p, b, "w0.15 baseline", sugar, bitter, mn9, orn_i, odors, kc, mbon)
    w1 = w0.copy(); w1[kk] = 0.0
    battery(Net(b.indptr, b.indices, w1, p), p, b, "w0.15 KC->KC=0", sugar, bitter, mn9, orn_i, odors, kc, mbon)
    for g in (2.0, 4.0):
        w2 = w1.copy(); w2[b.edges_between(apl, kc)] *= g
        battery(Net(b.indptr, b.indices, w2, p), p, b, f"w0.15 KC->KC=0 APL x{g:.0f}", sugar, bitter, mn9, orn_i, odors, kc, mbon)
    return 0
